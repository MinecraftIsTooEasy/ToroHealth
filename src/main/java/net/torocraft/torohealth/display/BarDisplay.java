...
public class BarDisplay {
  ...
  public BarDisplay(Minecraft mc, Object gui) {
  }
  ...
  public void draw(GuiGraphics guiGraphics, LivingEntity entity) {
    ...
    guiGraphics.drawString(minecraft.font, name, xOffset, 2, 0xFFFFFFFF, false);
    xOffset += minecraft.font.width(name) + 5;

    renderHeartIcon(guiGraphics, xOffset, 1);
    xOffset += 10;

    guiGraphics.drawString(minecraft.font, healthText, xOffset, 2, 0xFFFFFFFF, false);
    xOffset += minecraft.font.width(healthText) + 5;

    int armor = entity.getArmorValue();
    if (armor > 0) {
      renderArmorIcon(guiGraphics, xOffset, 1);
      xOffset += 10;
      guiGraphics.drawString(minecraft.font, entity.getArmorValue() + "", xOffset, 2, 0xFFFFFFFF, false);
    }
  }

  private void drawHealthBar(GuiGraphics guiGraphics, LivingEntity entity, int x, int y, int width) {
